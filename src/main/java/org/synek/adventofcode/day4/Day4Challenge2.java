...
public class Day4Challenge2 {
    ...
    public Day4Challenge2(String resource) {
    ...
    public static void main(String[] args) {
        int score = new Day4Challenge2(INPUT_RESOURCE).playGame();
    ...
    public int playGame() {
        for (int number : numbers) { // main loop of the game
            ...
            List<Board> completedBoards = new ArrayList<>();
            ...
            if (!completedBoards.isEmpty()) {
                boardList.removeAll(completedBoards);
            }
            if (boardList.isEmpty() && completedBoards.size() == 1) {
                return completedBoards.get(0).score() * number;
            }
        }
